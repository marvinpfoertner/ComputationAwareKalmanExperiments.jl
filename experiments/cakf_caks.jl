...
begin
	txs_plot = ColVecs(
		hcat(
			[
				[ts_plot[i], xs_plot[j]]
				for i in 1:length(ts_plot)
				for j in 1:length(xs_plot)
			]...
		)
	)
	...
	plot!(
		ts_plot,
		xs_plot,
		reshape(quantile.(p_marginals, .95), length(xs_plot), length(ts_plot)),
		st=:surface,
		fa=0.7,
	)
end
...
function plot_belief(ts, xs, states, H)
	plot(
		ts,
		xs,
		hcat([H * Statistics.mean(states[k]) for k in 1:length(ts)]...),
		st=:surface,
		colorbar=false,
	)
	...
	plot!(
		ts,
		xs,
		hcat([H * Statistics.mean(states[k]) + 1.96 * sqrt.(diag(H * Statistics.cov(states[k]) * H')) for k in 1:length(ts)]...),
		st=:surface,
		fa=0.7,
	)
end;
...
plot_belief(
	...
	xs_plot,
	[
		ComputationAwareKalman.interpolate(dgmp, fcache, t)
		for t in ts_plot
	],
	H_plot,
)
...
plot_belief(
	...
	xs_plot,
	[
		ComputationAwareKalman.interpolate(dgmp, fcache, scache, t)
		for t in ts_plot
	],
	H_plot,
)
