...
function (Σ::ExtrinsicSphericalMatern32)(λ₁θ₁, λ₂θ₂)
    ...
    # GCS -> Cartesian
    x₁ = (r_earth * cos(λ₁) * cos(θ₁), r_earth * sin(λ₁) * cos(θ₁), r_earth * sin(θ₁))
    x₂ = (r_earth * cos(λ₂) * cos(θ₂), r_earth * sin(λ₂) * cos(θ₂), r_earth * sin(θ₂))
...
function discretize_space(
    ...
    spatial_covmat = ComputationAwareKalmanExperiments.KernelMatrix{Float64}(
        stsgmp.spatial_cov_fn,
        λθs,
        λθs,
    )
