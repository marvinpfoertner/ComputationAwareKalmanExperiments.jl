...
Adapt.adapt_structure(to, Kₓ₁ₓ₂::KernelMatrix{T}) where {T} =
    KernelMatrix{T}(adapt(to, Kₓ₁ₓ₂.K), adapt(to, Kₓ₁ₓ₂.X₁), adapt(to, Kₓ₁ₓ₂.X₂))
...
function LinearAlgebra.mul!(
    Y_d::CuMatrix,
    Kₓ₁ₓ₂_d::ComputationAwareKalmanExperiments.KernelMatrix{
        <:Real,
        <:Any,
        <:CuVector,
        <:CuVector,
    ...
    CUDA.@sync begin
        @cuda threads = (nthreads_x, nthreads_y) blocks = (nblocks_x, nblocks_y) kernelmat_matmul_kernel!(
            Y_d,
            Kₓ₁ₓ₂_d.K,
            Kₓ₁ₓ₂_d.X₁,
            Kₓ₁ₓ₂_d.X₂,
...
function Base.:*(
    ...
    Y_d = CuMatrix{promote_type(eltype(Kₓ₁ₓ₂_d), eltype(V))}(
        undef,
        size(Kₓ₁ₓ₂_d.X₁, 1),
        size(V, 2),
    )
...
function LinearAlgebra.mul!(
    ...
    CUDA.@sync begin
        @cuda threads = nthreads blocks = nblocks kernelmat_matvec_kernel!(
            y_d,
            Kₓ₁ₓ₂_d.K,
            Kₓ₁ₓ₂_d.X₁,
            Kₓ₁ₓ₂_d.X₂,
